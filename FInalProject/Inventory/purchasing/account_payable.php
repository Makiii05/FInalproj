    ...
    <div class="container">
        <div class="col-md-6 py-3">
            <div class="profile-container" style="width: 1100px;">
                <h5 class="modal-title pb-3">Purchases to Pay</h5>

                    <?PHP
                    $result = $conn->query("select * from tblrequest where fldstatus = 'ordered'");
                    if(mysqli_num_rows($result) == 0){
                        ...
                        echo "<thead><tr align=center><th>Purchasing Officer</th><th>Purpose</th><th>Product</th><th>Details</th><th>Paid</th></tr></thead><tbody>";
                        while($row=$result->fetch_assoc()){
                            $prod = $row["fldproduct"];
                            $quan = $row["fldquantity"];
                            $uprice = $row["flduprice"];
                            $tprice = $row["fldtprice"];
                            $purp = $row["fldpurpose"];
                            ...
                            <td><a class='btn' href='../delivery/PHP_delivery.php?Paid=$row[id]&prod=$prod&quan=$quan&uprice=$uprice&purp=$purp'><i class='bi bi-check-circle-fill text-primary text-center'></i></a></td>
                            </tr>";
                        }
                        echo "</tbody></table>";
                    }
...
<!-- modal details -->
    ...
    if(isset($_GET["Paid"])){
        ...
        <script>
            ...
            function Hello(){
                alert('Purchase Paid. Added in Inventory.')
            }
        </script>";
    }
    ?>
